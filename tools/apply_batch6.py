"""Batch 6 applier: items[] formatindaki ceviri dosyasini tr.json'a uygular."""
import json
import shutil
from pathlib import Path

tr_path = Path("../translations/Content0/tr.json")
batch_path = Path("../translations/batch6_translated.json")
backup = tr_path.with_suffix(".json.bak_batch6")
...
CATEGORY_MAP = {
    ...
    "mainmenu": "mainmenu",
    "settings": "settings",
    "optioncatwalkfour": "optioncatwalkfour",
}
...
for item in batch["items"]:
    ...
    if not entry.get("category"):
        key_l = key.lower()
        for pre, cat in CATEGORY_MAP.items():
